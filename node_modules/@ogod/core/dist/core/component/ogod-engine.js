...
import { createEngine, destroyEngine, togglePauseEngine, startEngine } from "../redux/action.js";
import { ogodStore, ogodState$ } from "../redux/store.js";
import { createListenerOn, ReleasableLitElement } from "../utils.js";
import { map, filter, take } from "../../../../../rxjs/_esm5/operators/index.js";
let OgodEngineElement = class OgodEngineElement extends ReleasableLitElement {
  ...
  getEntity() {
    if (this.initScene) {
      this.loadMap = Object.assign(Object.assign({}, this.loadMap), {
  ...
  registerStart() {
    if (document.readyState === 'loading') {
      window.addEventListener('load', () => this.startEngine());
    } else {
      this.startEngine();
    }
  }
  ...
  startEngine() {
    ogodState$().pipe(map(state => state.engines[this.id]), filter(engine => engine && engine.entity.loaded), take(1)).subscribe(() => ogodStore.dispatch(startEngine(this.id)));
  }
