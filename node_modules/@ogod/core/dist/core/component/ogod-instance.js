...
import { createInstance, cloneInstance, addInstanceProperty, removeInstanceProperty, destroyInstance, removeFromScene, changeInstance, replaceInstanceProperty } from "../redux/action.js";
...
let OgodInstanceElement = class OgodInstanceElement extends ReleasableLitElement {
  ...
  async updated(changes, changed = {}) {
    if (ogodStore.getState().instances[this.name]) {
      if (changes.has('resource') && changes.get('resource') !== this.resource || changes.has('resources') && changes.get('resources') !== this.resources) {
        changed.resources = this.computeResources();
      }

      if (Object.keys(changed).length > 0) {
        ogodStore.dispatch(changeInstance(this.name, changed));
      }
    }
  }
  ...
  mutationsListener(mutations) {
    return from(mutations).pipe(filter(mutation => mutation.type === 'childList'), filter(mutation => mutation.removedNodes.length > 0 || mutation.addedNodes.length > 0), switchMap(mutation => {
      ...
      if (mutation.added && !oldProperties[el.name]) {
        ogodStore.dispatch(addInstanceProperty(this.name, el.name, {
          entity: el.getEntity(),
          runtime: el.getRuntime()
        }));
      } else if (!mutation.added && !this.properties[el.name]) {
        ogodStore.dispatch(removeInstanceProperty(this.name, el.name));
      } else {
        let entity = mutation.added ? el.getEntity() : this.properties[el.name].getEntity();
        let runtime = mutation.added ? el.getRuntime() : this.properties[el.name].getRuntime();
        ogodStore.dispatch(replaceInstanceProperty(this.name, el.name, {
          entity,
          runtime
        }));
      }
